...
u'' = -(u[j+1] - 2u[j] + u[j-1] )/h^2
du''/du[j+1] = -1/h^2
du''/du[j] = 2/h^2
du''/du[j-1] = -1/h^2

J mat : is a N*N matrix where N is the number of cells. which is symmetric and only three non-zero diagonals
...
PetscErrorCode FormJacobianLocal(DMDALocalInfo *info, PetscReal *u,
    ...
    for (i=info->xs; i<info->xs+info->xm; i++) {
        if ((i == 0) | (i == info->mx-1)) {
            v[0] = 1.0;
            PetscCall(MatSetValues(P,1,&i,1,&i,v,INSERT_VALUES));// set the non-zero value in the matrix
        } else {
            col[0] = i;
            v[0] = 2.0;
            if (!user->noRinJ) {
...
int main(int argc,char **args) {
  ...
  set correct term in the matrix about R(u) term in Jacobian
  ...
  */
  ...
  PetscCall(DMSetApplicationContext(da,&user));// set the user context in the DM

  PetscCall(DMCreateGlobalVector(da,&u));
  PetscCall(VecDuplicate(u,&uexact));
  PetscCall(DMDAVecGetArray(da,u,&au));// get the array from (au) of the vector
